...
def labelDictionary():
    ...
    cyrillic_upper = [chr(code) for code in range(0x0410, 0x042F + 1)]
    cyrillic_lower = [chr(code) for code in range(0x0430, 0x044F + 1)]
    additional_chars = ['Ё', 'ё', 'ғ', 'Қ', 'қ', 'Ү', 'Ө', 'ө', '–', '—', '…', ' ']
    ...
    _extend(latin_upper)
    _extend(latin_lower)
    _extend(cyrillic_upper)
    _extend(['Ё'])
    _extend(cyrillic_lower)
    _extend(['ё'])
    _extend(['ғ', 'Қ', 'қ', 'Ү', 'Ө', 'ө', '–', '—', '…'])
    _extend([' '])
...
tok = False
pad_start = char_classes
if not tok:
    tokens = {"PAD_TOKEN": pad_start}
else:
    tokens = {"GO_TOKEN": pad_start, "END_TOKEN": pad_start + 1, "PAD_TOKEN": pad_start + 2}
...
class WordLineDataset(Dataset):
    ...
    def __finalize__(self):
        ...
        # Include basefolder hash + metafile hash in cache key to avoid collisions
        import hashlib
        folder_hash = hashlib.md5(os.path.abspath(self.basefolder).encode()).hexdigest()[:8]
        meta_path = getattr(self, 'meta_file', '')
        meta_hash = hashlib.md5(os.path.abspath(meta_path).encode()).hexdigest()[:8] if meta_path else '00000000'
        save_file = '{}/{}_{}_{}_{}_{}.pt'.format(save_path, self.subset, self.segmentation_level, self.setname, folder_hash, meta_hash)
        ...
        # Build writer index for O(1) lookup in __getitem__ (avoids O(N) scan per item)
        from collections import defaultdict
        self._samples_by_writer = defaultdict(list)
        for item in self.data:
            self._samples_by_writer[item[2]].append(item)
